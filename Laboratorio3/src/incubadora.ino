...
// Falta switch de comunicacion serial, funcion de comunicacion serial y configuracion del controlador PID
...
int modo_serial; // Modo de operacion de la comunicacion serial, encendidio o apagado
...
void comunicacion_serial(int modo_serial){
  // Falta implementar
  if (modo_serial == 1) {
...
void loop() {
  ...
  pantalla_lcd(setpoint, input, output, leer_switch_pantalla);
  comunicacion_serial(leer_switch_serial);
}